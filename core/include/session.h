...
public:
  ...
  {
    message_in_thread_ = NULL;
    
    active_ = false;

    require_check_interval_ = boost::posix_time::milliseconds(500);
  }
...
private:
  ...
  void read_failed(const boost::system::error_code& error) {
    if (error == boost::system::errc::no_buffer_space) {
      ...
      BOOST_LOG_TRIVIAL(warning) << "[" << session_id_<<"] "<< __FUNCTION__ << " Overrun on receive buffer. Attempting to regain rx sync.";
    } else if (error) {
      // When some other read error has occurred, stop the session, which destroys
      // all known publishers and subscribers.
      BOOST_LOG_TRIVIAL(fatal) << "[" << session_id_<<"] "<< __FUNCTION__ << " Socket asio error, closing socket: " << error;
      if (is_active()) {
        stop();
      }
    }
  ...
  void write_message(Buffer& message, const uint16_t topic_id) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "[" << session_id_<<"] "<< __FUNCTION__<< " async_write: Sending buffer of " << length << " bytes to client.";
  ...
  void request_topics() {
    while(require_check_running_) {
      ...
      BOOST_LOG_TRIVIAL(debug) << "[" << session_id_<<"] "<< __FUNCTION__ << " Sending request topics message for VER=0xb9 protocol.";
