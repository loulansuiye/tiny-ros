...
class StringDataType(PrimitiveDataType):
    ...
    def serialize(self, f):
        ...
        f.write('      uint32_t length_%s = strlen(this->%s.c_str());\n' % (cn,self.name))
...
ROS_TO_EMBEDDED_TYPES = {
    ...
    'Header'  :   ('roslib_msgs::Header',  0, MessageDataType, ['roslib_msgs/Header'])
}
...
def roslib_copy_examples_files(path):
    if not os.path.exists(path+"applications/publisher"):
        os.makedirs(path+"applications/publisher")
    if not os.path.exists(path+"applications/subscriber"):
        os.makedirs(path+"applications/subscriber")
    if not os.path.exists(path+"applications/service"):
        os.makedirs(path+"applications/service")
    if not os.path.exists(path+"applications/service_client"):
        os.makedirs(path+"applications/service_client")
    files = ['applications/publisher/ExamplePublisher.cpp',
             'applications/subscriber/ExampleSubscriber.cpp',
             'applications/service/ExampleService.cpp',
             'applications/service_client/ExampleServiceClient.cpp']
